...
function App({ songs }) {
  ...
  return dom("div", { class: "music-player flex-column" }, dom(Slider, { slides: songs, handleChangeMusic: handleChangeMusic }), dom(Playlist, { list: songs, handleChangeMusic: handleChangeMusic }));
}
...
function Slider({ slides, handleChangeMusic }) {
  ...
  return dom(
    ...
    dom(
      ...
      dom("button", { class: "music-player__playlist-button center button" }, dom("i", { class: "icon-playlist" })),
      ...
      dom(
        ...
        slides.map(({ songName, files: { cover } }) => dom("img", { src: cover, class: "img", alt: songName }))
      )
    ),
    ...
    dom(
      ...
      dom(
        ...
        dom("div", null, dom("div", { class: "music-player__singer-name" }, dom("div", null, slides[0].artist))),

        dom("div", null, dom("div", { class: "music-player__subtitle" }, dom("div", null, slides[0].songName)))
      ),
      ...
      dom(
        ...
        dom("div", { class: "progress__wrapper" }, dom("div", { class: "progress__bar center" }))
      )
    )
  );
}
    ...
    list.map(({ songName, artist, files: { cover, song } }, index) => {
      return dom(
        "li",
        {
          ...
          onClick: () => handleChangeMusic({ isPrev: false, playListIndex: index }),
        },

        dom("div", { class: "flex-row _align_center" }, dom("img", { src: cover, class: "img music-player__song-img" }), dom("div", { class: "music-player__playlist-info  text_trsf-cap" }, dom("b", { class: "text_overflow" }, songName), dom("div", { class: "flex-row _justify_space-btwn" }, dom("span", { class: "music-player__subtitle" }, artist), dom("span", { class: "music-player__song-duration" })))),
...
function Loading() {
  return dom("div", { class: "loading flex-row" }, dom("span", { class: "loading__progress" }, "0"), dom("span", null, "%"));
}
    ...
    function downloadTheFiles(media, input) {
      return Promise.all(
        input.map((song) => {
          ...
          promise.then(() => {
            loadingProgress++;
            const progress = Math.round((loadingProgress / (songs.length * 2)) * 100);
